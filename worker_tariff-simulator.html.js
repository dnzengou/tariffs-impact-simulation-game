...
const agents = {
    ...
    'EU': {
        ...
        payoff: (action, oppActions) => {
            ...
            return lngCost + tradeLoss + (action.debtSwap ? -30 : 0);
        }
    },
    'EM': {
        strategies: {
        ...
        payoff: (action, oppActions) => {
            ...
            return action.tradeShift ? 25 : action.cryptoAdopt ? -10 : tariffHit / 4;
        }
    },
    'Crypto': {
        strategies: {
        ...
        payoff: (action, oppActions) => {
            const instability = (oppActions.US?.tariffs[0] || 0) + (oppActions.EU?.counterTariffs || 0);
...
const blocs = [
    { coords: [[0, 0], [5, 5]], tariffFactor: 0.5 },
    { coords: [[10, 10], [15, 15]], tariffFactor: 1.5 }
];
...
function updateGrid(actions) {
    ...
    for (let i = 0; i < GRID_SIZE; i++) {
        for (let j = 0; j < GRID_SIZE; j++) {
            ...
            const retaliationImpact = actions.EU.counterTariffs || 0;
            let neighborSum = 0;
...
function runSimulation({ numRounds, mistakeProb, usStrategy, euStrategy }) {
    const strategies = [
    ...
    const payoffHistory = { US: [], EU: [], EM: [], Crypto: [] };
    ...
    for (let round = 0; round < numRounds; round++) {
        strategies.forEach(agent => {
            ...
            if (round > 0 && Math.random() < 0.2 && agent.name !== 'US' && agent.name !== 'EU') {
    ...
    postMessage({ payoffs: accumulatedPayoffs, grid: Array.from(grid), payoffHistory });
}
